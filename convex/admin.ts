...
// ============================================
// OPERA EVENTS: Fetch from Supabase
// ============================================

export const getOperaEvents = query({
  args: {
    region: v.optional(v.union(v.literal("US"), v.literal("international"))),
  ...
  handler: async (ctx, args) => {
    ...
    let url = `${SUPABASE_URL}/rest/v1/opera_events?select=*`;
    
    if (args.region) {
      url += `&region=eq.${args.region}`;
    }
    if (args.country) {
      url += `&country=eq.${args.country}`;
    }
    
    url += `&order=event_date.asc`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch opera events: ${response.status}`);
    }
...
export const getOperaEventsCount = query({
  args: {
    region: v.optional(v.union(v.literal("US"), v.literal("international"))),
  },
  handler: async (ctx, args) => {
    ...
    let url = `${SUPABASE_URL}/rest/v1/opera_events?select=ticketmaster_id`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch opera events count: ${response.status}`);
    }
...
export const getOperaEventsByCountry = query({
  args: {},
  handler: async (ctx) => {
    const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL;
    const SUPABASE_KEY = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
    ...
    const url = `${SUPABASE_URL}/rest/v1/opera_events?select=country,region&order=country.asc`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch opera events by country: ${response.status}`);
    }

    const events = await response.json();
    
    // Group by country
    const counts: Record<string, { total: number; US: number; international: number }> = {};
    for (const event of events) {
      const country = event.country || 'Unknown';
      if (!counts[country]) {
        counts[country] = { total: 0, US: 0, international: 0 };
      }
      counts[country].total++;
      if (event.region === 'US') {
        counts[country].US++;
      } else {
        counts[country].international++;
      }
    }
