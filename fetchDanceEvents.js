...
const OUTPUT_DIR = path.join(__dirname, 'public', 'data', 'ticketmaster', 'Dance');
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'data.json');

const PAGE_SIZE = 200;
const MAX_PAGE = 4; // page * size < 1000, so max page = 4 (5 pages total)
...
async function main() {
  // Arts, Theater & Comedy → Dance
  const classifications = ['Dance'];
  let allEvents = [];

  for (const cls of classifications) {
    console.log(`\nFetching [${cls}]...`);
    const events = await fetchAllForClassification(cls);
