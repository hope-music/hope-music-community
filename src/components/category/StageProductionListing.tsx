...
interface StageProductionListingProps {
  category: string;
  items: ListingItem[];
  itemsPerPage?: number;
  basePath?: string;
...
export function StageProductionListing({
  category,
  items,
  itemsPerPage = ITEMS_PER_PAGE_DEFAULT,
  basePath,
}: StageProductionListingProps) {
  ...
  const getDetailHref = (itemId: string): string => {
    if (basePath) return `${basePath}/${itemId}`;
    return `/stage-production/${category}/${itemId}`;
  };
